...
v0.3.8은 0.3.6 Master를 밤새 켜 둔 현장 로그에서 확인된 중단을 고친 Master 변경입니다. 대화 기록이 길어져 대화창을 한 번 읽는 데 8초 가까이 걸리자, 회신 직전에 쓰는 화면 증거의 유효기간(15초)이 그 읽기 두 번(15.5~16.0초)보다 짧아졌고, 명령을 정확히 인식하고도 회신 직전에 중단되며 세션이 끝났습니다. 창을 옮기거나 무엇을 입력한 일은 없었습니다. v0.3.8은 이 유효기간을 한 번 읽기의 상한(15초)의 두 배에 5초를 더한 35초로 맞춥니다. 또한 실제 입력이 시작되기 전에 중단된 요청은 세션을 끝내는 대신 새 `Master Ready`를 보내 다시 대기합니다(연속 3회까지, 그 뒤에는 세션 종료). 입력이 시작된 뒤의 중단은 종전처럼 세션을 끝내며, 중단된 답장을 자동으로 다시 보내는 일은 어느 경우에도 없습니다. 재개되면 회신 없이 새 `Master Ready`만 도착하므로 명령을 다시 보내세요. 접수·전송·이력 규칙과 프로토콜 0.3.0은 그대로입니다. **실제 야간 운용에서는 아직 확인하지 않았습니다**(자동 검사만 통과). Slave는 버전 표기만 0.3.8로 올랐습니다.

v0.3.7은 0.3.6 Master의 첫 Win7 현장 로그가 남긴 항목별 시간에 대한 Master 변경입니다. 그 로그에서 대화창을 한 번 읽는 시간은 6.3~7.7초로 줄지 않았고, 그 가운데 약 80%가 형제 요소를 하나씩 건너뛰며 이동하는 데 쓰였습니다(메신저 창의 접근성 계층이 이동마다 같은 계층을 처음부터 다시 열거하는 구조). v0.3.7은 이 이동을 없애고 요소마다 자식 목록을 한 번의 질의로 받아 같은 순서로 읽습니다. 읽는 범위와 건너뛰기 판정, 접수·전송·이력 규칙과 프로토콜 0.3.0은 그대로이며, 회신에 쓰는 본문은 예전처럼 읽기 직전에 다시 확인합니다. **실제 속도 개선 폭은 아직 현장에서 확인하지 않았습니다**(자동 검사만 통과). Slave는 버전 표기만 0.3.7로 올랐습니다.

v0.3.6은 Win7 현장 로그에서 측정한 회신 속도를 줄이기 위한 Master 변경입니다. 대화창을 한 번 읽는 동안 노드마다 주고받던 UIA 호출 수를 줄이고, 답장 부분마다 반복하던 실행 파일 서명 검증을 같은 파일·같은 프로세스에 한해 한 번만 수행하며, Master 자신의 클릭이 만든 입력은 "PC 입력 정지" 대기에서 제외하고, 명령 대기 중에는 대화 기록 끝부분의 구조만 0.25초 간격으로 확인해 변화가 보이면 바로 화면을 읽습니다. 접수·전송·이력 규칙과 프로토콜 0.3.0은 그대로입니다. 현장 로그에서 서명 검증 재사용, 자기 클릭 제외, 명령 대기 중 구조 확인은 의도대로 동작했지만 화면을 한 번 읽는 시간 자체는 줄지 않았고, 그 원인이 v0.3.7의 변경으로 이어졌습니다. Slave는 버전 표기만 0.3.6으로 올랐습니다.
...
4. 메신저에서 `Master Ready. help, total status, pwrsi, watchdog 중 하나를 보내세요. [번호]`를 확인한 뒤 모바일에서 `pwrsi`를 보냅니다. 처리 시작 안내가 한 번 도착하며, 이후 추가 메시지는 대기열에 넣지 않고 무시합니다. 같은 보고서 번호의 마지막 `N/N`과 다음 `Master Ready`를 받은 뒤 새 명령을 보내세요. Ready 뒤의 대괄호 번호는 각 접수 회차를 구분하며 명령에 입력하지 않습니다.
...
| `help help`, `help total status`, `help pwrsi`, `help watchdog` | 해당 명령 도움말 |
| `total status` | Slave 기본 상태와 프로세스 목록 |
| `pwrsi` | 모든 PowerSI 인스턴스를 한 번 수집해 보고 |
| `watchdog on` | 지금 실행 중인 모든 PowerSI(시작 시각을 읽을 수 있는 것)를 완료 감시 목록에 추가. 프로세스 목록만 조회 |
| `watchdog on <PID>` | 해당 PID의 PowerSI 하나만 추가 (PID는 숫자, 앞자리 0 없음) |
| `watchdog off` | 모든 감시 해제 (Slave 조회 없음) |
| `watchdog off <PID>` | 해당 PID의 감시만 해제 |

명령은 새 메시지의 본문 전체가 일치해야 합니다. 답장에 포함된 명령 같은 문구는 실행하지 않습니다. `watchdog`은 PowerSI 완료 여부를 주기적으로 확인해 알리기만 하며, PowerSI 실행·종료와 임의 원격 명령은 제공하지 않습니다. `watchdog on`/`off`의 답장은 머리글 `WATCHDOG <번호>`의 한 메시지이고 처리 안내 없이 옵니다. 완료 알림과 경고(같은 대상이 3회 연속 판정 불가, Slave 조회 3회 연속 실패)는 새 번호의 `WATCHDOG <번호> | 완료 알림`/`| 경고` 메시지로 오며, 이미 보낸 알림은 다시 보내지 않습니다. 이 메시지들도 명령으로 실행하지 않습니다.
