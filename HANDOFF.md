...
갱신: 2026-09-22. Claude를 포함한 다음 검토자가 현재 구현과 검증 범위를 파악하기 위한 진입 문서입니다. 과거 계획보다 이 문서와 실제 소스, 사용자의 최신 지시를 우선합니다.
...
| 앱·설치파일 | **v0.3.8**, Master와 Slave 모두 동일 버전. 0.3.6 야간 현장 로그(G4)에서 답장 증거의 15초 유효기간이 길어진 대화의 스냅샷 2회보다 짧아 두 세션이 회신 직전에 끊긴 원인을 고치고(한도 35초), 실제 입력 전에 중단된 요청은 새 Ready로 다시 대기하게 한 Master 변경분이며 현장 미확인. Slave는 버전 표기만 올랐습니다 |
| 정식 배포 소스·태그 | 현재 정식 배포는 `v0.3.7` → `7e4d52a0b84caeeaed2842ef58bc2b4909f524f8` (PR #7 병합 커밋). 설치파일과 해시는 §9. 0.3.8 설치파일은 태그 `v0.3.8`의 CI 배포 실행이 발행하며, 0.3.7과 같은 절차로 공개 자산 해시를 확인한 뒤 §9에 기록합니다 |
| 이전 정식 배포 | v0.3.6 → `e5c5389798495af735077f6a644a0eb4a6ccb6cb`, v0.3.5 → `d27c2b8f57bd9b6078e4d8a085bd2bbe295d9f9a`, v0.3.4 → `932be45f293910ba4244c3c00a58e44343bdd704`, v0.3.3 → `e31a67c68ca33986016be00cf4990d8d81f9d8ec` |
| 통신 규약 | **0.3.0** — 앱 버전과 별개, 0.3.8에서도 변경 없음 |
| 호환성 | Master 0.3.8 + Slave 0.3.0~0.3.7. 0.2.x에서는 두 역할 모두 갱신 |
| 실행 환경 | Windows 7 SP1 Master / Windows 11 Slave, .NET Framework 4.8 |
| 이번 인계 범위 | v0.3.3 독립 소스 검토 결과의 반영과 v0.3.4 정식 배포, 첫 Win7 현장 로그(G1)에 대한 0.3.5 수정, v0.3.5 현장 로그의 속도 측정(G2)에 대한 0.3.6 변경, v0.3.6 현장 로그의 시간 분해(G3)에 대한 0.3.7 변경, v0.3.6 야간 로그의 증거 유효기간 초과(G4)에 대한 0.3.8 변경. 검토 기록은 [docs/REVIEW-2026-09-21-v0.3.3.md](docs/REVIEW-2026-09-21-v0.3.3.md) |
...
정식 배포는 `.github/workflows/ci.yml`의 `workflow_dispatch release_tag`로 수행합니다. 0.3.8 배포는 아직 없는 `release_tag=v0.3.8`로 실행합니다. **v0.3.3, v0.3.4, v0.3.5, v0.3.6, v0.3.7은 이미 존재하므로 다시 지정하지 않습니다.** 향후 코드 수정 배포는 새 앱 버전과 일치하는 미사용 `v<version>` 태그를 사용하고 기존 태그·자산을 교체하지 않습니다.
...
- **v0.3.8: 배포 후 기록.** 태그 `v0.3.8`의 CI 배포 실행이 설치파일 2개·ZIP 2개·`SHA256SUMS.txt`를 발행하면, 0.3.7과 같이 익명 다운로드로 SHA256과 GitHub digest 일치, ZIP 항목 허용 목록, 두 ZIP의 Slave 바이너리 동일, 두 EXE의 버전 문자열(0.3.8/규약 0.3.0)을 확인한 뒤 아래 형식으로 행을 추가합니다.
